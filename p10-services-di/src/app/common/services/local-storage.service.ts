...
import { ToastMessageType } from '../model/toast-message.model';
...
export class LocalStorageService {
  ...
  getLocal<T>(key: LocalStorageKey): T | null {
    ...
    try {
      ...
      this.toastMessageService.show(ToastMessageType.Error, `Error occurred while parsing local storage for key: ${key}`);
      return null;
    }
  }
  ...
  getSession<T>(key: LocalStorageKey): T | null {
    ...
    try {
      ...
      this.toastMessageService.show(ToastMessageType.Error, `Error occurred while parsing session storage for key: ${key}`);
      return null;
    }
  }
